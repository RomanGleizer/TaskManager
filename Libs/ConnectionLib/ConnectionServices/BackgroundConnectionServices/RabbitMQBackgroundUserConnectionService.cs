    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        consumer.Received += (model, ea) =>
        {
            ...
            var addNewTaskDesirializeData = JsonConvert.DeserializeObject<AddProjectToListOfUserProjectsResponse>(message)
                ?? throw new Exception("An error was occured while desirialize AddProjectToListOfUserProjectsResponse");

            using var scope = _serviceScopeFactory.CreateScope();

            var userService = scope.ServiceProvider.GetRequiredService<UserService>()
                ?? throw new Exception("An error was occured while getting UserService");
